...
*   **智能上下文情绪感知 (Auto-Context Emotion)**：突破单点指令控制。合成引擎可根据当前台词的局部前置语境（滑动窗口机制）自动推演自然的情绪过渡，产生 8 维平滑情绪概率向量注入音频潜空间，彻底解决角色连续对话中情绪突变、缺乏混合感受的痛点。支持轻量级本地微调模型（RoBERTa）推理与大型 LLM API 无缝热回退双模式。
...
对于后端的**智能上下文情绪感知**回退模式（在未部署本地轻量级模型时），需要在启动后端前配置以下环境变量：
```bash
export EMOTION_LLM_API_KEY="你的API Key"
export EMOTION_LLM_BASE_URL="你的API Base URL"      # 必须指向支持 OpenAI SDK 调用格式的地址，如 /v1
export EMOTION_LLM_MODEL="对应的模型名称"               # 例如 deepseek-chat, qwen-max 等
```
